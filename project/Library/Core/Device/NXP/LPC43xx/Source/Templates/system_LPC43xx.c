...
#define CLK_IRC           18000000UL//12000000UL    /* Internal oscillator frequency      */
...
void SystemInit_ExtMemCtl (void) {
#if (USE_EXT_STAT_MEM_CS0 || USE_EXT_DYN_MEM_CS0)
  ...
  LPC_SCU->SFSP1_15  = EMC_PIN_SET | 6;  /* P5_4:  D8   *********P1_15                       */
  LPC_SCU->SFSP1_16  = EMC_PIN_SET | 6;  /* P5_5:  D9 ****************P1_16                         */
  LPC_SCU->SFSP1_18  = EMC_PIN_SET | 7;  /* P5_6:  D10     ******P1_18                    */
  LPC_SCU->SFSP1_20  = EMC_PIN_SET | 7;  /* P5_7:  D11  ***********P1_20                       */
  ...
  LPC_EMC->CONFIG   = 0x00000000;       /* BIG-endian, Clock Ratio 1:1     */
  ...
  if (SystemCoreClock > 120000000UL) {
    ...
    __ISB();
    ((emcdivby2 )(emcdivby2_ram+1))
		(&LPC_CREG->CREG6, &LPC_CCU1->CLK_M4_EMCDIV_CFG, (1 << 5) | (1 << 2) | (1 << 1) | 1);
    for (n = 0; n < emcdivby2_szw; n++) 
		{
  ...
  LPC_EMC->DYNAMICCONFIG0    = (1 << 14) |  /* AM[14]   = 1//0                   */
                               (0 << 12) |  /* AM[12]   = 0                   */
                               (3 <<  9) |  /* AM[11:9] = 3                   */
                               (0 <<  7) ;  /* AM[8:7]  = 1                   */
	#endif														 
  ...
  LPC_EMC->DYNAMICRASCAS0    = 0x00000303;  /* Latency: RAS 3, CAS 3 CCLK cyc.RESET*/
  LPC_EMC->DYNAMICREADCONFIG = 0x00000001;  /* Command delayed by 1/2 CCLK  Configures the dynamic memory read strategy  */
//(ns) * ((freq)/((div)+1)))/1000000000
	LPC_EMC->DYNAMICRP         = EMC_NANOSEC ( 18, SystemCoreClock, div);//0.12ns  3CLK   //EMC_NANOSEC(ns, freq, div) (((uint64_t)(ns) * ((freq)/((div)+1)))/1000000000)
  LPC_EMC->DYNAMICRAS        = EMC_NANOSEC ( 48, SystemCoreClock, div);//0.264ns  4
  LPC_EMC->DYNAMICSREX       = EMC_NANOSEC ( 80, SystemCoreClock, div);//0.48ns
  LPC_EMC->DYNAMICAPR        = EMC_NANOSEC ( 35, SystemCoreClock, div);//0.21ns
  LPC_EMC->DYNAMICDAL        = EMC_NANOSEC ( 34, SystemCoreClock, div);//0.48ns   //
  LPC_EMC->DYNAMICWR         = EMC_NANOSEC ( 15, SystemCoreClock, div);//0.24ns
  LPC_EMC->DYNAMICRC         = EMC_NANOSEC ( 10, SystemCoreClock, div);//0.42ns  //60
  LPC_EMC->DYNAMICRFC        = EMC_NANOSEC ( 80, SystemCoreClock, div);//0.42ns
  LPC_EMC->DYNAMICXSR        = EMC_NANOSEC ( 80, SystemCoreClock, div);//0.48ns
  LPC_EMC->DYNAMICRRD        = EMC_NANOSEC ( 120, SystemCoreClock, div);//1.02ns
  LPC_EMC->DYNAMICMRD        = EMC_NANOSEC ( 10, SystemCoreClock, div);//0.24ns  //12
  ...
  LPC_EMC->DYNAMICCONTROL    = 0x00000183;  /* Issue NOP command              */
  WaitUs (1);
  LPC_EMC->DYNAMICREFRESH    = EMC_NANOSEC(  200, SystemCoreClock, div) / 16 + 1;//1.075ns
  WaitUs (10);
  LPC_EMC->DYNAMICREFRESH    = EMC_NANOSEC(15625, SystemCoreClock, div) / 16 + 1;//64mstREF
  WaitUs (10);
...
void SystemCoreClockUpdate (void) {
  ...
  uint32_t base_src = (LPC_CGU->BASE_M4_CLK ) ;
base_src=base_src>> 24;
base_src=base_src& 0x1F;
