...
#define GET_CAN_NO(CANx) ((CANx > LPC_C_CAN0) ? 0 : 1)
/* Macro for reading and writing to CCAN IF registers */
#define CAN_IF_Read(CANx, reg, IFsel) (CANx->##IFsel##_##reg)
#define CAN_IF_Write(CANx, reg, IFsel, val) (CANx->##IFsel##_##reg=val)

/* Macro for writing IF to specific RAM message object */  
#define CAN_IF_readBuf(CANx,IFsel,msg) \
  CANx->##IFsel##_##CMDMSK_W=RD|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB; \
  CANx->##IFsel##_##CMDREQ=msg; \
  while (CANx->##IFsel##_##CMDREQ & IFCREQ_BUSY );
  
/* Macro for reading specific RAM message object to IF */
#define CAN_IF_writeBuf(CANx,IFsel,msg) \
  CANx->##IFsel##_##CMDMSK_W=WR|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB; \
  CANx->##IFsel##_##CMDREQ=msg; \
  while (CANx->##IFsel##_##CMDREQ & IFCREQ_BUSY );
#else
...
MSG_CB TX_cb0, TX_cb1;
MSG_CB RX_cb0, RX_cb1;


message_object recv_buff[2];
 ...
 * @return      None
 **********************************************************************/
void CAN_RxInt_MessageProcess(LPC_C_CANn_Type* CANx, uint8_t msg_no )
{
  uint32_t msg_id;
  uint32_t *p_add;
  uint32_t reg1, reg2;
  ...
  CAN_IF_readBuf(CANx, IF2, msg_no);             /* Read the message into the IF registers */

  p_add = (uint32_t *)&recv_buff[GET_CAN_NO(CANx)];

  if ( CAN_IF_Read(CANx,ARB2, IF2) & ID_MTD ) {  /* bit 28-0 is 29 bit extended frame */
    /* mask off MsgVal and Dir */
    reg1 = CAN_IF_Read(CANx,ARB1, IF2);
    reg2 = CAN_IF_Read(CANx,ARB2, IF2);
    msg_id = (reg1|(reg2<<16));
  } else {
    /* bit 28-18 is 11-bit standard frame */
    msg_id = (CAN_IF_Read(CANx,ARB2, IF2) &0x1FFF) >> 2;
  }

  p_add[0] = msg_id;
  p_add[1] = CAN_IF_Read(CANx, MCTRL, IF2) & 0x000F;	/* Get Msg Obj Data length */
  p_add[2] = (CAN_IF_Read(CANx, DA2, IF2) << 16) | CAN_IF_Read(CANx, DA1, IF2);
  p_add[3] = (CAN_IF_Read(CANx, DB2, IF2) << 16) | CAN_IF_Read(CANx, DB1, IF2);

  /* Clear interrupt pending bit */
  CAN_IF_Write(CANx, MCTRL, IF2, UMSK | RXIE | EOB | CAN_DLC_MAX);
  /* Save changes to message RAM */
  CAN_IF_writeBuf(CANx,IF2, msg_no);
 ...
 * @return      None
 **********************************************************************/
void CAN_TxInt_MessageProcess(LPC_C_CANn_Type* CANx, uint8_t msg_no )
{
  /* Clear interrupt pending bit */
  CAN_IF_Write(CANx, MCTRL, IF2, UMSK|RXIE|EOB|CAN_DLC_MAX);
  /* Save changes to message RAM */
...
void CAN_IRQHandler(LPC_C_CANn_Type* CANx)
{
  uint32_t canstat = 0;
  uint32_t can_int, msg_no;

  while ( (can_int = CANx->INT) != 0 ) {         /* While interrupt is pending */
    canstat = CANx->STAT;                        /* Read CAN status register */
    ...
    if ( can_int & CAN_STATUS_INTERRUPT ) {
      ...
      if ( (canstat & STAT_LEC) == 0 ) {         /* NO ERROR */
        ...
        if ((msg_no >= 1 ) && (msg_no <= 16)) {
          ...
          if ((1 << (msg_no-1)) != CANx->ND1) {
            /* message object does not contain new data! */
            ND1ErrCnt++;
            break;
          }
          CAN_RxInt_MessageProcess(CANx, msg_no);
	  if (CANx == LPC_C_CAN1) {
            RX_cb1(msg_no);
	  } else {
            RX_cb0(msg_no);
          ...
          CANx->STAT &= ~STAT_TXOK;
          CAN_TxInt_MessageProcess(CANx, msg_no);
	  if (CANx == LPC_C_CAN1) {
            TX_cb1(msg_no);
	  } else {
            TX_cb0(msg_no);
 ...
 * @return      None
 **********************************************************************/
void CAN_Init(LPC_C_CANn_Type* CANx, uint32_t BitClk, CCAN_CLKDIV_Type ClkDiv , MSG_CB Tx_cb, MSG_CB Rx_cb)
{
  if (CANx == LPC_C_CAN1) {
    TX_cb1 = Tx_cb;
  } else {
  ...
  if ( !(CANx->CNTL & CTRL_INIT) ) {
    /* If it's in normal operation already, stop it, reconfigure
       everything first, then restart.  */
    CANx->CNTL |= CTRL_INIT;                     /* Default state */
  }

  CANx->CLKDIV = ClkDiv;                         /* Divider for CAN VPB3 clock */
  CANx->CNTL |= CTRL_CCE;                        /* Start configuring bit timing */
  CANx->BT = BitClk;
  CANx->BRPE = 0x0000;
  CANx->CNTL &= ~CTRL_CCE;                       /* Stop configuring bit timing */

  CANx->CNTL &= ~CTRL_INIT;                      /* Initialization finished, normal operation now. */
  while ( CANx->CNTL & CTRL_INIT );

  /* By default, auto TX is enabled, enable all related interrupts */
  CANx->CNTL |= (CTRL_IE|CTRL_SIE|CTRL_EIE);
  return;
}
 ...
 * @return      None
 **********************************************************************/
void CAN_Send(LPC_C_CANn_Type* CANx, uint8_t msg_no, uint32_t *msg_ptr )
{
  ...
  if (Length > CAN_DLC_MAX) {
    Length = CAN_DLC_MAX;
  }
  CAN_IF_Write(CANx, MCTRL, IF1, UMSK|TXIE|TXRQ|EOB|RMTEN|(Length & DLC_MASK));
  CAN_IF_Write(CANx, DA1, IF1, *msg_ptr);        /* Lower two bytes of message pointer */
  CAN_IF_Write(CANx, DA2, IF1, (*msg_ptr++)>>16);/* Upper two bytes of message pointer */
  CAN_IF_Write(CANx, DB1, IF1, *msg_ptr);        /* Lower two bytes of message pointer */
  CAN_IF_Write(CANx, DB2, IF1, (*msg_ptr)>>16);  /* Upper two bytes of message pointer */

  /* Configure arbitration */
  if(!(tx_id & (0x1 << 30))) {                   /* bit 30 is 0, standard frame */
    /* Mxtd: 0, Mdir: 1, Mask is 0x7FF */
    CAN_IF_Write(CANx, MSK2, IF1, MASK_MDIR | (ID_STD_MASK << 2));
    CAN_IF_Write(CANx, MSK1, IF1, 0x0000);

    /* MsgVal: 1, Mtd: 0, Dir: 1, ID = 0x200 */
    CAN_IF_Write(CANx, ARB1, IF1, 0x0000);
    CAN_IF_Write(CANx, ARB2, IF1, ID_MVAL| ID_DIR | (tx_id << 2));
  } else {
    /* Mxtd: 1, Mdir: 1, Mask is 0x7FF */
    CAN_IF_Write(CANx, MSK2, IF1, MASK_MXTD | MASK_MDIR | (ID_EXT_MASK >> 16));
    CAN_IF_Write(CANx, MSK1, IF1, ID_EXT_MASK & 0x0000FFFF);

    /* MsgVal: 1, Mtd: 1, Dir: 1, ID = 0x200000 */
    CAN_IF_Write(CANx, ARB1, IF1, tx_id & 0x0000FFFF);
    CAN_IF_Write(CANx, ARB2, IF1, ID_MVAL|ID_MTD | ID_DIR | (tx_id >> 16));
  }

  /* Write changes to message RAM */
  CAN_IF_writeBuf(CANx, IF1, msg_no);
 ...
 * @return      None
 **********************************************************************/
void CAN_Recv(LPC_C_CANn_Type* CANx, uint8_t msg_no, uint32_t *msg_ptr, Bool RemoteEnable)
{
  ...
  if (!(rx_id & (0x1<<30))) { /* standard frame */
    ...
    CAN_IF_Write(CANx, MSK1, IF1, 0x0000);
    CAN_IF_Write(CANx, MSK2, IF1, ID_STD_MASK << 2);
    /* MsgVal: 1, Mtd: 0, Dir: 0 */
    CAN_IF_Write(CANx, ARB1, IF1, 0x0000);
    CAN_IF_Write(CANx, MCTRL, IF1, rmten|UMSK|RXIE|EOB|CAN_DLC_MAX);
    CAN_IF_Write(CANx, DA1, IF1, 0x0000);
    CAN_IF_Write(CANx, DA2, IF1, 0x0000);
    CAN_IF_Write(CANx, DB1, IF1, 0x0000);
    CAN_IF_Write(CANx, DB2, IF1, 0x0000);
    CAN_IF_Write(CANx, ARB2, IF1, ID_MVAL | ((rx_id) << 2));
    /* Transfer data to message RAM */
    CAN_IF_writeBuf(CANx,IF1, msg_no);
  } else {
    rx_id &= (0x1<<30)-1 ; /* Mask ID bit */
    /* Mxtd: 1, Mdir: 0, Mask is 0x1FFFFFFF */
    CAN_IF_Write(CANx, MSK1, IF1, ID_EXT_MASK & 0xFFFF);
    CAN_IF_Write(CANx, MSK2, IF1, MASK_MXTD | (ID_EXT_MASK >> 16));
    /* MsgVal: 1, Mtd: 1, Dir: 0 */
    CAN_IF_Write(CANx, ARB1, IF1, (rx_id) & 0xFFFF);
    CAN_IF_Write(CANx, MCTRL, IF1, rmten|UMSK|RXIE|EOB|CAN_DLC_MAX);
    CAN_IF_Write(CANx, DA1, IF1, 0x0000);
    CAN_IF_Write(CANx, DA2, IF1, 0x0000);
    CAN_IF_Write(CANx, DB1, IF1, 0x0000);
    CAN_IF_Write(CANx, DB2, IF1, 0x0000);
    CAN_IF_Write(CANx, ARB2, IF1, ID_MVAL | ID_MTD | ((rx_id) >> 16));
    /* Transfer data to message RAM */
    CAN_IF_writeBuf(CANx,IF1, msg_no);
  }
  return;
 ...
 * @return      None
 **********************************************************************/
void CAN_ReadMsg(LPC_C_CANn_Type* CANx, uint32_t msg_no, message_object* buff){
  int i;

  buff->id = recv_buff[GET_CAN_NO(CANx)].id;
  buff->dlc = recv_buff[GET_CAN_NO(CANx)].dlc;
  if (recv_buff[GET_CAN_NO(CANx)].dlc > CAN_DLC_MAX) {
    recv_buff[GET_CAN_NO(CANx)].dlc = CAN_DLC_MAX;
  }
  for (i = 0; i < recv_buff[GET_CAN_NO(CANx)].dlc; i++) {
    buff->data[i] = recv_buff[GET_CAN_NO(CANx)].data[i];
  }
}
