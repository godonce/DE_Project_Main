...
uint8_t (*_db_get_char_nonblocking)(LPC_USARTn_Type *UARTx);
#if defined(HITEX_LCD_TERM)
...
uint8_t UARTGetCharInNonBlock(LPC_USARTn_Type *UARTx)
{	
	uint8_t tmp = 0;
	UART_Receive(UARTx, &tmp, 1, NONE_BLOCKING);
	return(tmp);
}
/*********************************************************************//**
...
void debug_frmwrk_init_clk(uint32_t Clock_Speed)
{
	 ...
	 * Initialize UART1 pin connect
	 */
	scu_pinmux(0xA ,1 , MD_PDN, FUNC3); 	// PC.13 : UART2_TXD
	scu_pinmux(0xA ,2 , MD_PLN|MD_EZI|MD_ZI, FUNC3); 	// PC.14 : UART2_RXD
