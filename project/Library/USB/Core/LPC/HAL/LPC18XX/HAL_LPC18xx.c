...
CGU_SetDIV(CGU_CLKSRC_IDIVA, 3);
		CGU_EntityConnect(CGU_CLKSRC_PLL1, CGU_CLKSRC_IDIVA); /* FIXME Run base BASE_USB1_CLK clock from PLL1 (assume PLL1 is 60 MHz, no division required) */
CGU_EntityConnect(CGU_CLKSRC_IDIVA, CGU_BASE_USB1);
CGU_GetPCLKFrequency(CGU_PERIPHERAL_USB1);
		 /* Turn on the phy */
	    LPC_CREG->CREG0 &= ~(1<<5);
...
#if defined(USB_CAN_BE_HOST)
	USB_REG(USBPortNum)->USBCMD_H = 1<<1;
	/* wait for reset to complete */
	while (USB_REG(USBPortNum)->USBCMD_H & 1<<1);

	/* Program the controller to be the USB device controller */
	USB_REG(USBPortNum)->USBMODE_H =   (0x3<<0)| (1<<4)| (1<<3) ;
	if(USBPortNum==0)
	{
